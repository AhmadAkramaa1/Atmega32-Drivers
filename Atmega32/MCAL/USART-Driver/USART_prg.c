...
void USART_VoidInit(void)
{
	...
	/**UCSRCVarLoc = 0b10000000;                              */
	
	#if ((MODE == SYNC_MASTER) || (MODE == SYNC_SLAVE))
		SET_BIT(UCSRCVarLoc,6);
	#else //Mode is async
		CLEAR_BIT(UCSRCVarLoc,6);
	#endif
	
	#if (PARITY == NO)
		CLEAR_BIT(UCSRCVarLoc,5);
		CLEAR_BIT(UCSRCVarLoc,4);
	#elif (PARITY == EVEN)
		SET_BIT(UCSRCVarLoc,5);
		CLEAR_BIT(UCSRCVarLoc,4);
	#elif (PARITY == ODD)
		SET_BIT(UCSRCVarLoc,5);
		SET_BIT(UCSRCVarLoc,4);	
	#else //default is NO
		CLEAR_BIT(UCSRCVarLoc,5);
		CLEAR_BIT(UCSRCVarLoc,4);	
	#endif
	...
	#if (STOPBITS == _2SPB)
		SET_BIT(UCSRCVarLoc,3);
	#else //default is 1 stop Bit
		CLEAR_BIT(UCSRCVarLoc,3);
	#endif
	
	/** frame format */
	#if (DATABITS == _9BITS)
		SET_BIT(UCSRB,2);
		SET_BIT(UCSRCVarLoc,2);
		SET_BIT(UCSRCVarLoc,1);
	#elif (DATABITS == _5BITS)
		CLEAR_BIT(UCSRB,2);
		CLEAR_BIT(UCSRCVarLoc,2);
		CLEAR_BIT(UCSRCVarLoc,1);
	...
	/** clk polarity bit when synchronous mode only */
			...
			SET_BIT(UCSRCVarLoc,0);
		#else //Default will be SAMPLERX_DOWN_CHANGETX_UP
			CLEAR_BIT(UCSRCVarLoc,0);
		#endif
	#else //Async
		CLEAR_BIT(UCSRCVarLoc,0);
	#endif
	/** write the value directly to UCSRC */
	UCSRC = UCSRCVarLoc;
...
/****************************************************************************/
/** Note - SET_BIT,CLEAR_BIT,GET_BIT...etc,                                 */
/** works as follows : read Register, modify then write.                    */
/** incase of UCSRC we must only write so we will be                        */
/** reading and modifying UBRRH if we used them directly.                   */
/**************                                                             */
/** -Use a variable(UCSRCVarLoc) to store the value then write UCSRC.       */
/** -Use Concatination : UCSRC=USARTCONC(1,SYNC,parity,StopBits,Data,Ucpol);*/
/** write each macro with right number of bits in priv.h                    */
/** and use conc help method for it to work                                 */ 
/****************************************************************************/
